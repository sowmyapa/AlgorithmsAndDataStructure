...
public class MinimumAverageWaitingTime {
    ...
    class ArrivalTimeComparator implements Comparator<Customer>{
    ...
    class WaitingTimeComparator implements Comparator<Customer>{
    ...
    PriorityQueue<Customer> arrivalTime;
    PriorityQueue<Customer> waitingTime;


    public static void main(String[] args){
        MinimumAverageWaitingTime minimumAverageWaitingTime = new MinimumAverageWaitingTime();

        minimumAverageWaitingTime.determineAverageWaitingTime();
    }

    private void determineAverageWaitingTime() {
        Scanner in = new Scanner(System.in);
        int numberOfCustomers = in.nextInt();
        in.nextLine();

        Comparator<Customer> arrivalComparator = new ArrivalTimeComparator();
        Comparator<Customer> waitingComparator = new WaitingTimeComparator();

        arrivalTime = new PriorityQueue<>(arrivalComparator);
        waitingTime = new PriorityQueue<>(waitingComparator);


        for(int i=0;i<numberOfCustomers;i++){
            String customerDetail = in.nextLine();
            String[] splitDetail = customerDetail.split(" ");
            Customer c = new Customer(Integer.parseInt(splitDetail[0]),Integer.parseInt(splitDetail[1]));
            arrivalTime.add(c);
            //waitingTime.add(c);
        ...
        long currentTime=0;
          while(arrivalTime.size()>0 || waitingTime.size()>0){
              if (waitingTime.size()>0){
                 Customer c = waitingTime.poll();
                  totalWaitingTime+=(currentTime+c.waitingTime-c.arrivalTime);
                  currentTime+=c.waitingTime;
              }
              while(arrivalTime.size()>0 && arrivalTime.peek().arrivalTime<=currentTime){
                  waitingTime.add(arrivalTime.poll());
              }
              if(waitingTime.size()==0 && arrivalTime.size()>0){
                  Customer c = arrivalTime.poll();
                  currentTime=c.arrivalTime+c.waitingTime;
                  totalWaitingTime+=c.waitingTime;

              }

          }
          System.out.println(totalWaitingTime/numberOfCustomers);
